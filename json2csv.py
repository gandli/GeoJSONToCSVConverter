...
def convert_geojson_to_csv(city_geojson_file, district_geojson_dir, csv_file):
    ...
    for index, feature in city_gdf.iterrows():
        ...
        # 读取区县的 GeoJSON 文件
        district_geojson_file = os.path.join(district_geojson_dir, f"{district_id}.geojson")
        if os.path.exists(district_geojson_file):
            ...
            for d_index, d_feature in district_gdf.iterrows():
                ...
                # 添加乡镇信息
                rows.append([township_name, city_level + 2, township_hierarchy, 
                             json.dumps({"type": "Point", "coordinates": list(township_center.coords[0])}), 
